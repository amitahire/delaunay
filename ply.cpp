...
int main(int argc, char **argv)
{
	...
	if(ply.Read(fp))
	{
		...
		{
			printf("element %s found with %d values!\n", vertexReader.GetName(), vertexReader.GetCount());
			...
			{
				int c = std::min(10, vertexReader.GetCount());
				for(int i = 0; i < c; ++i)
				{
				...
				if(c < vertexReader.GetCount()) 
					printf(" ... ... ...\n");
			}
			else printf("Couldn't find x y or z\n");
		}

		PlyData::ElementReader faceReader = ply.GetElement("face");
		if(faceReader.Valid())
		{
			printf("element %s found with %d values!\n", faceReader.GetName(), faceReader.GetCount());
			int idxId = faceReader.GetPropertyId("vertex_indices");
			if(idxId >= 0)
			{
				int c = std::min(10, faceReader.GetCount());
				for(int i = 0; i < c; ++i)
				{
					...
					{
						int val = faceReader.GetPropertyListValue<int>(idxId, i, j);
						printf("%d ", val);
					}
					printf("\n");
				}
				if(c < faceReader.GetCount())
					printf("... ... ...\n");
			}
